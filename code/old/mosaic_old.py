...
def resize_image(image: Image, size: tuple) -> np.ndarray:
    '''Takes an image and resizes to a given size (width, height) as passed to the size parameter '''
    
    resized_image = ImageOps.fit(image, size, Image.LANCZOS, centering=(0.5, 0.5))
    return np.array(resized_image)

# Current directory to handle images locations
CWD = os.getcwd()

start_time = time.time()
print('Starting...')

image_in_location = os.path.join(CWD, 'ImgTest.jpg')
image_in = load_image(image_in_location)
...
mos_template = image_in[::(height//target_res[0]),::(height//target_res[1])] ## Create a mosaic template
...
images = []
dataset_location = os.path.join(CWD, 'dataset', 'DIV2K_train_LR_bicubic', 'X2', '*')
mosaic_size = (40, 40) ## Defines size of each mosiac image

## Load images from the dataset as numpy arrays
for file in glob.glob(dataset_location):
    im = load_image(file)
    images.append(im)
...
image_out = Image.new('RGB', (mosaic_size[0]*target_res[0], mosaic_size[1]*target_res[1])) ## Create a new image to store the final mosaic
...
for i in range(target_res[0]):
    for j in range(target_res[1]):
        ...
        image_out.paste(im, (x,y))

image_out_location = os.path.join(CWD, 'result.jpg')
image_out.save(image_out_location)
